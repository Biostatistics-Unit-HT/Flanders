...
def main():
    ...
    parser.add_argument("--liftover", type=str, default = False, help="Option for liftover")
    ...
    if args.liftover:
        if len(set(table_files[["grch"]]))>1:
            print("Error: GRCh builds are not unique.", file=sys.stderr)
            return sys.exit(1)
    ...
    for index, record in table_files.iterrows():
        ...
        if os.path.exists(gwas_path):
            ...
            if os.path.exists(bfile_path + ".bim") and os.path.exists(bfile_path + ".bed") and os.path.exists(bfile_path + ".fam"):
                ...
                if set(gwas_col).issubset(record):
                    continue
                else:
